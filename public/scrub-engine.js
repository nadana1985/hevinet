  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (i === 0) cop = after ? 0 : smooth(1 - pr / 0.62);            // greets on landing
        ...
        if (before) cop = 0;
        else if (y > seg.end) cop = smooth(1 - (y - seg.end) / (0.4 * vh));
    ...
    // Fade out fixed layers when scrolling past the scroll-world section end
    const lastSeg = SECTIONS[N - 1]._seg;
    const exitProgress = clamp((y - lastSeg.end) / (0.5 * vh), 0, 1);
    const globalOpacity = 1 - exitProgress;
    stage.style.opacity = globalOpacity;
    copylayer.style.opacity = globalOpacity;
    route.style.opacity = globalOpacity;
    scrollbar.style.opacity = globalOpacity;
    sky.style.opacity = globalOpacity;
    stage.style.pointerEvents = globalOpacity > 0.1 ? 'auto' : 'none';
    copylayer.style.pointerEvents = globalOpacity > 0.1 ? 'auto' : 'none';
    route.style.pointerEvents = globalOpacity > 0.1 ? 'auto' : 'none';
...
function injectCSS() {
  ...
  const css = `
  .sw-root{--sw-bg:#090d16;--sw-ink:#ffffff;--sw-ink-soft:#cbd5e1;--sw-accent:#eab308;
  ...
  html,body{margin:0;background:var(--sw-bg,#090d16);overflow-x:hidden;}
  .sw-sky{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none;background:var(--sw-bg);}
  .sw-sky__grad{position:absolute;inset:-10%;background:linear-gradient(178deg,rgba(9,13,22,0.95) 0%,#090d16 55%,rgba(9,13,22,0.9) 100%);}
  .sw-sky__glow{position:absolute;inset:0;background:radial-gradient(60% 42% at 74% 16%,color-mix(in srgb,var(--sw-accent) 15%,transparent),transparent 70%);}
  ...
  .sw-pt--dot::before{background:radial-gradient(circle at 34% 30%,color-mix(in srgb,var(--sw-accent) 60%,#fff),#fff 82%);}
  ...
  .sw-brand{display:flex;align-items:center;gap:10px;text-decoration:none;color:#ffffff;}
  ...
  .sw-brand__name{font-family:var(--sw-font-display);font-weight:700;font-size:1.1rem;color:#ffffff;}
  .sw-nav{display:flex;gap:4px;padding:5px;background:rgba(15,23,42,0.65);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.15);border-radius:999px;}
  .sw-nav__item{font:inherit;font-size:.82rem;color:#cbd5e1;border:0;background:transparent;cursor:pointer;padding:7px 14px;border-radius:999px;transition:color .25s,background .25s;}
  .sw-nav__item:hover{color:#ffffff;} .sw-nav__item.is-active{color:#000000;font-weight:700;background:var(--sw-accent);}
  .sw-topcta{text-decoration:none;font-weight:600;font-size:.9rem;color:#000000;background:var(--sw-accent);padding:10px 20px;border-radius:999px;white-space:nowrap;box-shadow:0 4px 14px rgba(234,179,8,0.3);}
  .sw-stage{position:fixed;inset:0;z-index:10;pointer-events:none;}
  .sw-scene{position:absolute;inset:0;opacity:0;overflow:hidden;will-change:opacity;}
  .sw-scene__video,.sw-scene__still{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 42%;}
  .sw-scene__still{will-change:transform;} .sw-scene.has-clip .sw-scene__still{opacity:0;} .sw-scene__video{z-index:1;}
  .sw-copylayer{position:fixed;inset:0;z-index:20;pointer-events:none;}
  .sw-copylayer::before{content:"";position:absolute;inset:0;width:min(60vw,820px);background:linear-gradient(90deg, rgba(9,13,22,0.85) 0%, rgba(9,13,22,0.6) 45%, rgba(9,13,22,0.15) 75%, transparent 100%);}
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:50%;transform:translateY(-50%);width:min(44vw,480px);opacity:0;will-change:opacity,transform;background:rgba(15,23,42,0.55);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.12);padding:32px;border-radius:24px;box-shadow:0 20px 50px rgba(0,0,0,0.5);}
  .sw-copy__num{font-family:ui-monospace,Menlo,monospace;font-size:.78rem;letter-spacing:.14em;color:#94a3b8;font-weight:600;}
  .sw-copy__eyebrow{display:block;margin-top:10px;font-family:var(--sw-font-display);font-weight:700;font-size:.84rem;letter-spacing:.16em;text-transform:uppercase;color:var(--sw-accent);}
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:800;color:#ffffff;font-size:clamp(1.8rem,3.8vw,2.8rem);line-height:1.08;margin:10px 0 0;letter-spacing:-.01em;text-shadow:0 4px 16px rgba(0,0,0,0.8);}
  .sw-copy__body{margin-top:14px;font-size:clamp(.95rem,1.15vw,1.05rem);line-height:1.55;color:#e2e8f0;max-width:40ch;text-shadow:0 2px 10px rgba(0,0,0,0.8);}
  .sw-copy__tags{list-style:none;display:flex;flex-wrap:wrap;gap:8px;margin:20px 0 0;padding:0;}
  .sw-copy__tags li{font-size:.8rem;font-weight:600;color:#ffffff;padding:6px 14px;border-radius:999px;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.18);backdrop-filter:blur(8px);}
  .sw-copy__cta{display:flex;flex-wrap:wrap;gap:12px;margin-top:24px;pointer-events:auto;}
  .sw-btn{text-decoration:none;font-weight:600;font-size:.95rem;padding:13px 24px;border-radius:999px;transition:transform .2s, background .2s, box-shadow .2s;}
  .sw-btn--primary{color:#090d16;background:#ffffff;font-weight:700;box-shadow:0 4px 16px rgba(255,255,255,0.2);} .sw-btn--primary:hover{transform:translateY(-2px);background:#f8fafc;box-shadow:0 6px 20px rgba(255,255,255,0.3);}
  .sw-btn--ghost{color:var(--sw-ink);border:1.5px solid color-mix(in srgb,var(--sw-ink) 25%,transparent);background:rgba(255,255,255,0.05);} .sw-btn--ghost:hover{transform:translateY(-2px);background:rgba(255,255,255,0.12);}
